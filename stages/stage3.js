...
var STAGE_SPIKE_LAYOUT = [
  { x: 3000, w: 96, triggerX: 2870 },
  { x: 4510, w: 64, triggerX: 4420 },
];
...
function getPlatformAtX(x) {
  ...
  // If no exact match, find the closest platform
  var closest = null;
  var minDist = Infinity;
  for (var i = 0; i < MAP.platforms.length; i++) {
    var p = MAP.platforms[i];
    var pCenterX = p.x + p.w / 2;
    var dist = Math.abs(x - pCenterX);
    if (dist < minDist) {
      minDist = dist;
      closest = p;
    }
...
function buildMap() {
  ...
  MAP.spikes = [];
  STAGE_SPIKE_LAYOUT.forEach(function (spikeConfig) {
    var platform = getPlatformAtX(spikeConfig.x);
    if (platform) {
      MAP.spikes.push({
        x: spikeConfig.x,
        y: platform.y,
        w: spikeConfig.w,
        triggerX: spikeConfig.triggerX,
        active: false,
        riseTimer: 0,
      });
    }
