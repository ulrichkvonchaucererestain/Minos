(function () {
  ...
  function askQuestion(item) {
    ...
    if (idx === q.answer) {
      if (
        window.PL &&
        typeof window.PL.stamina === "number" &&
        typeof window.STAM_MAX !== "undefined"
      ) {
  ...
  function getItemsForStage() {
    ...
    itemState = defs.map(function (def, i) {
      var p =
        MAP.platforms[Math.min(def.platform, MAP.platforms.length - 1)] ||
        MAP.platforms[0];
      var px =
        p.x + Math.max(22, Math.min((p.w || 140) - 54, 34 + (i % 2) * 42));
      var py = p.y - 40;
      return {
        type: def.type,
        icon: def.icon,
